...
//    static /*SparseArray*/ Map<Integer, Card> cards =  /*SparseArray*/ [:]
    ...
    static init() {
        ...
        //之后放到DB中去
        heros.put(100001, createHero("圣骑", 0, "这是个圣骑", "圣光即正义", 0, 0, 30, 100001))

        lgd "初始化所有Hero结束"
    }

    static Hero createHero(String title, int patternId, String description = "暂无", String explain = "", int attack = 1, Integer armor, int blood = 1, int skillId = 100001) {
        def skill = SkillBuilder.newSkill(skillId)
        new Hero(title: title, patternId: patternId, description: description, explain: explain,
                attack: new MInt(attack),armor: new MInt(armor),  blood: new MInt(blood), skill: skill)
    }
